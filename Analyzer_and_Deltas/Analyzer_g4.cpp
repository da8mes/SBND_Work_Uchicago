...
//=================Electron Hists=====================================//
  ...
  // delta mc vs theory stack 
  THStack* deltaStack = new THStack("deltaStack", "Delta Probability Dist");  

  // theory delta expectations 
  TH1F* deltas = new TH1F("deltas", "Delta Distribution", 100, 0, 1);
...
for (gallery::Event ev(filename) ; !ev.atEnd(); ev.next()) {
  ...
  // computeDeltas(deltas, allMuonStats)

  // Normalize delta plots
  Double_t mcScale = 1/eEnergyDelta->Integral();
  eEnergyDelta->Scale(mcScale);
  eEnergyDelta->DrawCopy(); // not sure about this
 
  Double_t theoryScale = 1/deltas->Integral();
  deltas->Scale(theoryScale);
  deltas->DrawCopy(); 

  // Add to stackDelta
  deltaStack->Add(eEnergyDelta);  
  deltaStack->Add(deltas); 
